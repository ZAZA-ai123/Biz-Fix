...
import { quotes as initialQuotes, type Quote } from "@/lib/mock-data";
...
export default function QuotesPage() {
  const [rows, setRows] = useState<Quote[]>(() => [...initialQuotes]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const matchDate = inDateRange(r.created, dateFrom, dateTo);
      if (!q) return matchStatus && matchDate;
      const hay = `${r.id} ${r.client} ${r.project}`.toLowerCase();
                  ...
                  <tbody className="divide-y divide-border/60">
                    {filtered.map((q) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 font-medium text-foreground">{q.client}</td>
